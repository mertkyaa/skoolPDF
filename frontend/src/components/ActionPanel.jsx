...
const TOOL_HINTS = {
  ...
  msoffcrypto: { label: 'msoffcrypto-tool', cmd: 'pip3 install msoffcrypto-tool' },
};
...
export default function ActionPanel({ onBack }) {
  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      if (currentTask === 'merge') {
        ...
        switch (currentTask) {
          ...
          case 'protect':
            ...
            endpoint = '/protect';
            outputExt = '.' + (files[0].name.split('.').pop() || 'pdf');
            break;
      ...
      triggerDownload(new Blob([res.data], { type: contentType }), match ? match[1] : `${baseName(files[0])}${outputExt}`);
      setSuccess(true);
